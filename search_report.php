...
<form class="contact_form" action="search.php" method="post" name="contact_form">
	...
	<?php
		...
		<tr><td><strong>Total Nets issued to Pregnant women(ANC):<strong></td>
				...
				<?php
					$results = mysql_query("SELECT sum(ANC) FROM net_issue WHERE customer_code LIKE'%$key%' OR MFL_code LIKE'%$key%' OR hospital_name LIKE'%$key%' OR month LIKE'%$key%' ");
				...
				<td style="border-left: 1px solid #C1DAD7;">
				<?php
					$results = mysql_query("SELECT sum(CWC) FROM net_issue WHERE customer_code LIKE'%$key%' OR MFL_code LIKE'%$key%' OR hospital_name LIKE'%$key%' OR month LIKE'%$key%' ");
							while($row2 = mysql_fetch_array($results))
							  {
							   $gggg=$row2['sum(CWC)'];
							   echo $gggg;
				...
				<td style="border-left: 1px solid #C1DAD7;">
				<?php
						$results = mysql_query("SELECT sum(CWC), sum(ANC) FROM net_issue WHERE customer_code LIKE'%$key%' OR MFL_code LIKE'%$key%' OR hospital_name LIKE'%$key%' OR month LIKE'%$key%' ");
							while($row2 = mysql_fetch_array($results))
							  {
							   $gggg=$row2['sum(CWC)'];
							   $cccc=$row2['sum(ANC)'];
